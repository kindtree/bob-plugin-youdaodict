# 词阶 WordTier — Bob 学英语词典插件

[![tests](https://github.com/kindtree/bob-plugin-wordtier/actions/workflows/test.yml/badge.svg)](https://github.com/kindtree/bob-plugin-wordtier/actions/workflows/test.yml)
[![release](https://img.shields.io/github/v/release/kindtree/bob-plugin-wordtier)](https://github.com/kindtree/bob-plugin-wordtier/releases/latest)
[![license](https://img.shields.io/github/license/kindtree/bob-plugin-wordtier)](./LICENSE)

**按你的学习级别个性化的 [Bob](https://bobtranslate.com/) 学英语词典插件**。划词查**英文单词**时返回完整词典(释义/发音/例句/近义/词组/英释/词频/考试标签);**1-4 个汉字**反向查英文候选(按词性分组,可点);**整句中文**返回翻译并把句中词按 CET4/CET6/雅思/托福/GRE 等 **10 档等级分组**(蓝字可点跳查),还能让 LLM **倾向你目标级别的词汇**(选雅思就给雅思级译法,选 CET4 就给四级级译法)。

核心词典功能无需任何 key,直接用有道公开接口;整句翻译 + 等级分组为可选解锁,配 DeepSeek API key 启用。
...
Bob 自带的金山词霸经常提示"没有查到此单词",尤其是六级以上、考研、专业领域词。**词阶**(WordTier)换用有道作为数据源,词库覆盖明显更全;同时把发音、例句、英释、相关词、考试标签等都一次性渲染出来,看一次词等于查了好几本词典。

进一步,**词阶**这个名字对应它的核心差异化:**按你的学习级别给你看词**。你选目标级别(雅思 / CET6 / 高中 / GRE…),整句翻译里的内容词会按等级分组渲染;只显示你关心的范围,把视线集中在该学的词上,而不是被一堆 the/is/a 这种简单词淹没。
...
1. 到 [Releases](https://github.com/kindtree/bob-plugin-wordtier/releases/latest) 下载最新的 `wordtier.bobplugin`(v1.x 老版本文件名仍是 `youdaodict.bobplugin`,GitHub 自动重定向)
2. 双击安装,Bob 偏好设置 → 服务 中启用"词阶 WordTier"
3. 把它拖到翻译服务列表靠前的位置,这样划词会优先用它

或从源码自行打包:

```bash
git clone https://github.com/kindtree/bob-plugin-wordtier.git
cd bob-plugin-wordtier
bash build.sh   # 产出 wordtier.bobplugin,双击即可安装
```
...
这个插件还在打磨,欢迎在 [Issues](https://github.com/kindtree/bob-plugin-wordtier/issues) 里:
