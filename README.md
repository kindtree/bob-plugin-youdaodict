...
- **(可选)整句翻译 + 按考试等级分组(v1.7+,需 DeepSeek key)**:开启后任意中文句都能 LLM 翻译,并把句中词按 `小学 / 初中 / 高中 / CET4 / CET6 / 考研 / 雅思 / 托福 / GRE` 共 10 档分组渲染,**学习者能一眼看出哪些词是自己目标考试范围**
- **(v1.8+)按你的学习级别个性化**:在设置里选自己正在攻克的级别(雅思 / 考研 / CET6 等),LLM 翻译会**倾向使用该级别的常考词汇**,且只显示你关心的范围(默认"目标级别及以上"),把视线集中在该学的词上
...
- **核心词典功能零 key**:英文单词查词、中文短词反查、jsonapi 词典模式整句翻译,全部直接打有道公开接口,不需要登录、不需要 npm
- **可选解锁(v1.7+)**:整句翻译启用 DeepSeek 后(用户填 key)能处理任意中文句,并按考试等级分组词;不开启时保持纯免费/免 key
- 不上传任何东西到第三方分析平台
